...
define("DL_MIXED", "mezclado");
	...
	public function validar( $lista = [] ) {
		// Se detiene la ejecución del código si no se ha seleccionado un método de envío:
		if ( empty(trim( $this -> method )))
			return;
		
		// Capturar valores en función de la instanciación de la clase:
		...
		// Se detiene la ejecución del código si las listas que se van
		// a comparar no tiene datos o algunas de ellas no tienen datos:
		if ( count($this -> list ) <= 0 || count($lista) <= 0 )
			return false;

		if ( $this -> compare( $this -> list, $lista ) )
			return true;
	}
